...
@Path("/depgraph")
public class DependencyGraphResolutionService {

    private static final Logger LOG = LoggerFactory.getLogger(DependencyGraphResolutionService.class);

    private IMavenResolver resolver;

    @Inject
    public DependencyGraphResolutionService(IMavenResolver resolver) {
    ...
    public Response resolveDependents( //
        ...
        try {
            ...
            return Response.status(sc(EXPECTATION_FAILED), e.getMessage()).build();
        } catch (Exception e) {
            logError("resolveDependents", Set.of(gav(groupId, artifactId, version)), config, e);
            return Response.status(sc(INTERNAL_SERVER_ERROR), e.getMessage()).build();
        }
    }

    private static int sc(Status expectationFailed) {
        return expectationFailed.getStatusCode();
    }
    ...
    private static ResolverConfig getConfig(Long resolveAt, String depth, Integer limit, Scope scope, Boolean alwaysIncludeProvided, Boolean alwaysIncludeOptional) {

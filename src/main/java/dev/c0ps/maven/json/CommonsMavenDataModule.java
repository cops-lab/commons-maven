...
import dev.c0ps.maven.collections.ImmutableEmptyLinkedHashSet;
import dev.c0ps.maven.data.Dependency;
import dev.c0ps.maven.data.Exclusion;
import dev.c0ps.maven.data.Ids;
...
public class CommonsMavenDataModule extends SimpleModule {

    private static final Logger LOG = LoggerFactory.getLogger(CommonsMavenDataModule.class.getName());
    ...
    public CommonsMavenDataModule() {
        ...
        addSerializer(VersionConstraint.class, new JsonSerializer<VersionConstraint>() {
            @Override
            public void serialize(VersionConstraint value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
                gen.writeString(value.getSpec());
            }
        });
        addDeserializer(VersionConstraint.class, new JsonDeserializer<VersionConstraint>() {
            @Override
            public VersionConstraint deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
        ...
        addSerializer(Exclusion.class, new JsonSerializer<Exclusion>() {
            @Override
            public void serialize(Exclusion value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
        ...
        addDeserializer(Exclusion.class, new JsonDeserializer<Exclusion>() {
            @Override
            public Exclusion deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
        ...
        addSerializer(DefaultArtifactVersion.class, new JsonSerializer<DefaultArtifactVersion>() {
            @Override
            public void serialize(DefaultArtifactVersion value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
                gen.writeString(value.toString());
            }
        });
        addDeserializer(DefaultArtifactVersion.class, new JsonDeserializer<DefaultArtifactVersion>() {
            @Override
            public DefaultArtifactVersion deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
    ...
    private static class PomDeserializer extends JsonDeserializer<Pom> {
        ...
        public Pom deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
            ...
            pb.dependencyManagement = addChildren(node, p, "dependencyManagement", () -> new HashSet<Dependency>(), NO_DEP_MGMT);
    ...
    private static Set<VersionConstraint> readVersionConstraints(boolean isShortFormat, DeserializationContext ctxt, JsonNode node) {
    ...
    private static <CollectionT extends Collection<Dependency>> CollectionT addChildren(JsonNode node, JsonParser p, String key, Supplier<CollectionT> prod, CollectionT defaultVal)
